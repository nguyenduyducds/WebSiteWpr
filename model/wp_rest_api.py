...
class WordPressRESTClient:
    ...
    def __init__(self, site_url, username, password):
        ...
        retry_strategy = Retry(
            total=3,
            backoff_factor=1,
            status_forcelist=[429, 502, 503, 504], # Bỏ 500 do WP thường sinh ra 500 khi timeout nhưng post đã tạo
            allowed_methods=["HEAD", "GET", "PUT", "DELETE", "OPTIONS"] # KHÔNG auto-retry POST (tránh trùng lặp)
        )
        adapter = HTTPAdapter(max_retries=retry_strategy)
        self.session.mount("https://", adapter)
        self.session.mount("http://", adapter)
        self.session.verify = False  # Bỏ qua lỗi SSL cert hết hạn
    ...
    def create_post(self, title, content, featured_media_id=None, status='publish', categories=None, tags=None, **kwargs):
        """
        ...
        try:
            ...
            post_data = {
                'title': title,
                'content': content,
                'status': final_status,  # Gử i thẳng status thực tế, không cần 2 bước
            }
            ...
            try:
                # Gửi lệnh Tạo Nháp khá nhanh
                response = self.session.post(
                    self.posts_endpoint,
                    json=post_data,
                    headers=headers,
                    timeout=15  # 15s là dư sức cho Draft
                )
            except requests.exceptions.ReadTimeout:
                print(f"[REST_API] ⚠️ Server chạy tốn thời gian (kể cả với lệnh Tạo Nháp). Dừng chờ để tránh lặp..")
                return False, None, None
            ...
            if response.status_code in [200, 201]:
                ...
                # Nếu server trả về status khác publish (do có plugin can thiệp), thử force publish lần 2
                if final_status == 'publish' and data.get('status') != 'publish':
                    print(f"[REST_API] ⚡ Status={data.get('status')}, đang force publish lần 2...")
                    try:
                        ...
                        if pub_resp.status_code in [200, 201]:
                            post_url = pub_resp.json().get('link', post_url)
                            print(f"[REST_API] ✅ Force publish thành công!")
                    except Exception as pub_err:
                        print(f"[REST_API] ⚠️ Force publish lỗi: {pub_err}")
    ...
    # TAXONOMY HELPERS
    # ─────────────────────────────────────────────────────────────────
    def get_categories(self, per_page=100) -> list:
        ...
        try:
            ...
            print(f"[REST_API] get_categories error: {e}")
        return []

    def create_category(self, name: str, description: str = "") -> dict:
        """Tạo category mới, trả về dict có 'id'."""
        try:
            ...
            if resp.status_code in (200, 201):
                return resp.json()
            print(f"[REST_API] create_category failed: {resp.status_code} {resp.text[:200]}")
        except Exception as e:
            print(f"[REST_API] create_category error: {e}")
        return {}

    def get_tags(self, per_page=100) -> list:
        ...
        try:
            ...
            print(f"[REST_API] get_tags error: {e}")
        return []

    def create_tag(self, name: str) -> dict:
        """Tạo tag mới, trả về dict có 'id'."""
        try:
            slug = re.sub(r"[^a-z0-9]+", "-", name.lower()).strip("-")
            resp = self.session.post(
                f"{self.api_base}/tags",
            ...
            if resp.status_code in (200, 201):
                return resp.json()
            print(f"[REST_API] create_tag failed: {resp.status_code} {resp.text[:200]}")
        except Exception as e:
            print(f"[REST_API] create_tag error: {e}")
        return {}

    def upload_media(self, file_path: str, alt_text: str = "") -> dict:
        """
        Upload ảnh/video file lên WordPress media library.
        Trả về dict có 'id' và 'source_url'.
        """
        try:
            if not os.path.isfile(file_path):
                print(f"[REST_API] upload_media: file not found: {file_path}")
                return {}
            import mimetypes
            ...
            if resp.status_code in (200, 201):
                info = resp.json()
                if alt_text and "id" in info:
                    self.session.post(
                        f"{self.api_base}/media/{info['id']}",
                        json={"alt_text": alt_text},
                        auth=HTTPBasicAuth(self.username, self.password),
                        timeout=10, verify=False
                    )
                return info
            print(f"[REST_API] upload_media failed: {resp.status_code} {resp.text[:200]}")
        except Exception as e:
            print(f"[REST_API] upload_media error: {e}")
        return {}
