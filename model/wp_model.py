...
class BlogPost:
    ...
    def _generate_content_fallback(self):
        ...
        if self.raw_content and self.raw_content.strip():
            ...
            # FEATURED IMAGE: JNews theme auto-displays featured_media at post top.
            # But JNews generates og:image from the FIRST <img> in content body, not from featured_media.
            # Solution: Add a hidden img tag at the start so JNews picks it for og:image,
            # without causing visible duplication on the page.
            content_parts = []
            if self.image_url and self.image_url.strip():
                print(f"[WP_MODEL] Adding hidden og:image hint for JNews (no visible duplication)")
                # Hidden image: JNews PHP parser finds this as first <img> for og:image
                # aria-hidden + clip ensures it's invisible to users but parseable by server-side code
                hidden_img = f'<img src="{self.image_url}" width="1" height="1" alt="{self.title}" style="position:absolute;clip:rect(0,0,0,0);overflow:hidden;" aria-hidden="true" />'
                content_parts.append(hidden_img)
            
            # Add user's raw content
            content_parts.append(self.raw_content)
            content = "\n".join(content_parts)
            self.content = content
            
            # VIDEO: Insert at the very top (before all text content)
            # User wants: Featured Image -> Video -> Text content
            if self.video_url:
                print(f"[WP_MODEL] Generating video block for top position...")
                video_block = self._generate_video_block(self.video_url)
                if video_block:
                    # Prepend video BEFORE all text content
                    self.content = video_block + "\n\n" + self.content
                    print(f"[WP_MODEL] Video inserted at TOP of content!")
                else:
...
'''
        ...
        # 0. FEATURED IMAGE: Hidden og:image hint for JNews
        # JNews auto-displays featured_media, but generates og:image from first content <img>
        if self.image_url and self.image_url.strip():
            print(f"[WP_MODEL] Adding hidden og:image hint for JNews (auto-gen path)")
            hidden_img = f'<img src="{self.image_url}" width="1" height="1" alt="{self.title}" style="position:absolute;clip:rect(0,0,0,0);overflow:hidden;" aria-hidden="true" />'
            content_parts.append(hidden_img)
        
        # 1. VIDEO EMBED - placed at the top (right after featured image)
        # User wants: Featured Image -> Video -> Text
        if self.video_url:
            video_block = self._generate_video_block(self.video_url)
            if video_block:
                content_parts.append(video_block)
                print(f"[WP_MODEL] Added video block at TOP of auto-generated content")
        
        # 2. INTRODUCTION Section with styled heading
...
class WPAutoClient:
    ...
    def post_article(self, blog_post, reuse_selenium_client=None, reuse_fast_client=None):
        ...
        try:
            ...
            # If REST is blocked (HTML responses), use wp-admin form posting via requests (fast, no Selenium UI).
            if getattr(rest_client, "rest_blocked", False):
                print(f"[WP_AUTO] 🚫 REST blocked ({getattr(rest_client, 'rest_blocked_reason', 'blocked')}), using wp-admin post form API")
                try:
                    ok, post_id, post_url = rest_client.create_post_via_admin_form(
                        title=blog_post.title,
                        content=blog_post.content,
                        status="publish",
                        featured_media_id=getattr(blog_post, 'featured_media_id', None),
                    )
                    if ok:
                        return True, post_url or f"{rest_client.site_url}/?p={post_id}"
                    print("[WP_AUTO] ⚠️ wp-admin post form failed, falling back to Selenium method...")
                except Exception as admin_err:
                    print(f"[WP_AUTO] ⚠️ wp-admin post form error: {admin_err}")
                    print("[WP_AUTO] Falling back to Selenium method...")
                # Mark to skip REST and go straight to Selenium
                skip_to_selenium = True
            
            if not skip_to_selenium:
            ...
            # METHOD 2: Fallback to Selenium (if REST API failed/unavailable/blocked)
            print("[WP_AUTO] ========================================")
